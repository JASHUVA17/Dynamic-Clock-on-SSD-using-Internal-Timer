 ...
 * Author: Jashuva
 *
 * Created on 23 July, 2024, 7:57 AM
 */
...
void init_config() {
    ...
    TRISA = TRISA & 0xF0; // Configure Port A for specific purposes
    SSD_CNT_PORT = SSD_CNT_PORT & 0xF0; // Initialize SSD control port
    SSD_DATA_PORT = 0x00; // Clear SSD data port
    TRISC = TRISC | INPUT_PINS; // Set Port C pins as input
}
...
unsigned char read_digital_keypad(unsigned char detection_type) {
    static unsigned char once = 1; // Static variable to track key press state

    if (detection_type == STATE_CHANGE) {
        if (((KEY_PORT & INPUT_PINS) != ALL_RELEASED) && once) {
            once = 0; // Detect a state change in key press

            return (KEY_PORT & INPUT_PINS); // Return the key press value
        } else if ((KEY_PORT & INPUT_PINS) == ALL_RELEASED) {
            once = 1; // Reset the state if all keys are released
        }
    } else if (detection_type == LEVEL) {
        return (KEY_PORT & INPUT_PINS); // Return the current key press level
    }
